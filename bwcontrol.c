...
@code
	select pg_add_ingest_table('public', 'test_table', 1, 1, '', 0);
		INFO:  Success(9)
		 pg_add_ingest_table
		---------------------
		 Success(9)
 ...
 * @param[in] whole_column : whether or not to add whole columns of the given table
...
pg_add_ingest_table(PG_FUNCTION_ARGS)
{
	...
	int nwhole_column = PG_GETARG_INT32(5);
