...
//int generate_contents(char *contents, char *topics)
//{
//	int ret = EJSON;
//	char *req_json = NULL;
//	json_error_t parse_err;
//	json_t *resp_json = NULL, *jtopics = NULL;
//
//	elog(INFO, "%s, %s",contents, topics);
//	strcpy(contents, "[\"A\", {\"B\": \"C\"}, 1, 2, 3]");
//	resp_json = json_loadb(contents, strlen(contents), 0, &parse_err);
//	if(!resp_json){
//		goto CLEANUP;
//	}
//
...
//	}
//
//	jtopics = json_string(topics);
//	if(!jtopics){
//		goto CLEANUP;
//	}
//
//	if(json_object_set(resp_json, "topics", jtopics) < 0){
//		goto CLEANUP;
//	}
//
//	req_json = json_dumps(resp_json, JSON_COMPACT);
//	if(!resp_json){
//		goto CLEANUP;
//	}
//
//	strncpy(contents, req_json, strlen(req_json));
//	ret = 0;
//
//CLEANUP:
//	xpfree(req_json);
